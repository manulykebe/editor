...
import { ChevronRight, ChevronDown, FileText, Folder, Plus, FolderPlus, File, X } from 'lucide-react';
...
const FileTreeItem: React.FC<{ item: FileTreeItem; onRefresh: () => void }> = ({ item, onRefresh }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newPath = `${item.path}/${newItemName}${newItemType === 'file' ? '.js' : ''}`;
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`http://localhost:3000/api/files?path=${encodeURIComponent(item.path.slice(1))}`, {
        method: 'DELETE'
      });
      onRefresh();
    } catch (error) {
  ...
  return (
    <div>
      <div className="group flex items-center hover:bg-gray-700 text-sm">
        ...
        <button
          className="hidden group-hover:block p-1 hover:bg-gray-600 rounded mr-2"
      ...
      {isCreating && (
        <form onSubmit={handleSubmit} className="ml-8 my-1 flex items-center">
          <input
            type="text"
            value={newItemName}
